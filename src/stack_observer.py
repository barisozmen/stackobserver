...
def stack_key(frame):
    return frame.f_code.co_filename.split('/')[-1] + ':' + str(frame.f_code.co_firstlineno) + ' ' + str(frame.f_code.co_name).replace('<', '&lt;').replace('>', ' &gt;')
...
class StackObserver(ContextDecorator):
    ...
    def __exit__(self, *exc):
        ...
        subprocess.run(['python', 'src/stack_html_prettifier.py', str(self.inspected_file_path)])
        return False
        ...
        def trace_fn(frame, event, arg):
            ...
            if event == 'call':
                ...
                output(f"""<div class="stack" id="{stack_id}" stack_key="{stack_key(frame)}"
                            ...
                            f_code_co_firstlineno="{frame.f_code.co_firstlineno}"
                    >
